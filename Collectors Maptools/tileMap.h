...
enum SELECTMODE
{
	...
	SEL_SMOBJ,		//샘플오브젝트 선택
	SEL_DELOBJ,		//오브젝트 삭제
	SEL_SMSPEED,	//타일이   gif의 경우 애니메이션 속도조절
	SEL_FILL,		//타일     채우기
	SEL_SAVE,		//타일     저장
	SEL_LOAD		//타일     불러오기
};
...
private:
	...
	tagButton		_responeBtn;		//리스폰 설정버튼


	tagButton		_selSmSetBtn;		//샘플 버튼의 지형 구조 저장을 위하여
	tagButton		_selTileSetBtn;		//타일 그리기용 버튼
	tagButton		_selOBJSetBtn;		//오브젝트 그리기용 버튼

	
	tagButton		_selSmTileGRASS;		//샘플 버튼의 지형 구조 저장을 위하여
	tagButton		_selSmTileWATER;		//타일 그리기용 버튼
	tagButton		_selSmTileEND;			//오브젝트 그리기용 버튼
	tagButton		_selSmTileFrameSpeed;	//샘플타일의 프레임 속도조절용




	tagButton		_selSmObjNONE;			//샘플 타일에서 오브젝트 설정을 위하여		논
	tagButton		_selSmObjBLOCK;			//샘플 타일에서 오브젝트를 설정하기 위하여 블록

	tagButton		_selTileMap[3];			//타일 3종류를 선택하기 위하여
...
public:
	...
	void smTileInit();		//가져올 타일 샘플 이미지 초기화
	void btnInit();			//세이브 로드 등 버튼 초기화
	...
	void backGroundDraw();	//배경 그림 집합 틀도 포함
	void tilesDraw();		//타일관련 그림 집합 그릴타일,샘플타일
	void btnDraw();			//버튼 그림 집합
	//Render관련 함수 모집	END
	...
	//선택  START
	void tileSetting();
	void tileSelect();		//샘플타일로부터 정보를 가져와서 타일에 칠하는 것
	void smTileSelect();	//샘플타일의 프레임을 정해주는 함수
	void smTerrainSet();	//샘플타일의 타일 속성을 정해주는 함수
	void smObjSet();
	void smTileChange();
	void smSpeedSet();
